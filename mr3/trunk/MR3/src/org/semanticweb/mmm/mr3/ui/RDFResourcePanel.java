...
public class RDFResourcePanel extends JPanel implements ListSelectionListener {
    ...
    class URIPanel extends JPanel {
        ...
        public boolean isErrorResource() {
            ...
            if (isAnonBox.isSelected()) { return false; }
            if (isLocalDuplicated(uri) || gmanager.isEmptyURI(uri)
                    || gmanager.isDuplicatedWithDialog(uri, cell, GraphType.RDF)) { return true; }
            return false;
        }
    ...
    class DecideActon extends AbstractAction {
        ...
        private void apply() {
            if (uriPanel.isErrorResource()) { return; }
            ...
            if (typePanel.isType()) {
                ...
                if (resTypeCell != null) {
                    ...
                    if (!gmanager.getCurrentRDFEditor().isEditMode()) {
                        gmanager.selectClassCell(resTypeCell); // 対応するクラスを選択する
                    }
                }
            } else {
            ...
            setCellValue();
            gmanager.selectRDFCell(cell); // AttributeDialogの表示をRDFリソースに戻す
