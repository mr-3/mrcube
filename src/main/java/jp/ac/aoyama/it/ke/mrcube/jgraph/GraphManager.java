...
import jp.ac.aoyama.it.ke.mrcube.io.RDFSModelExtraction;
import jp.ac.aoyama.it.ke.mrcube.layout.GraphLayoutData;
...
public class GraphManager {
    ...
    public static MR3Constants.CellViewType cellViewType;
    ...
    public InstanceEditor getInstanceEditor() {
        return mr3ProjectPanel.getInstanceEditor();
    }
    ...
    public RDFGraph getInstanceGraph() {
        if (mr3ProjectPanel != null) {
            return (RDFGraph) mr3ProjectPanel.getInstanceEditor().getGraph();
        } else {
            return null;
        }
    }
    ...
    public Set<GraphCell> findRDFResourceSet(String keyword) {
        ...
        RDFGraph rdfGraph = getInstanceGraph();
    ...
    private void setVisibleEditors(boolean t) {
        getInstanceEditor().setVisible(!t);
    ...
    public void clearSelection() {
        getInstanceGraph().clearSelection();
    ...
    public void addTypeCells() {
        ...
        for (Object rdfCell : getInstanceGraph().getAllCells()) {
    ...
    public void removeTypeCells() {
        Object[] rdfCells = getInstanceGraph().getAllCells();
        ...
        getInstanceGraph().removeCellsWithEdges(typeCellList.toArray());
    }

    private boolean isRDFResourceDuplicated(String uri, Object cell, MR3Constants.GraphType type) {
        Object[] rdfCells = getInstanceGraph().getAllCells();
        for (Object rdfCell1 : rdfCells) {
            ...
            if (rdfCell instanceof InstanceCell) {
                ...
                if (tmpURI.equals(uri) && rdfCell != cell) {
                    ...
                    return (!(type == MR3Constants.GraphType.Class && rdfsModel.getURI().equals(RDFS.Class) || type == MR3Constants.GraphType.Property
    ...
    public boolean isDuplicatedWithDialog(String uri, Object cell, MR3Constants.GraphType type) {
     ...
     * 名前空間のリストを返す
     */
    public Set<String> getNameSpaceSet(MR3Constants.GraphType type) {
        if (type == MR3Constants.GraphType.Instance) {
            return getRDFNameSpaceSet();
        } else if (type == MR3Constants.GraphType.Class) {
            return getClassNameSpaceSet();
        } else if (type == MR3Constants.GraphType.Property) {
    ...
    public Object getRDFResourceCell(Resource uri) {
        Object[] cells = getInstanceGraph().getAllCells();
        for (Object cell1 : cells) {
    ...
    public void selectRDFCell(Object cell) {
        selectCell(cell, getInstanceGraph());
    }
    ...
    private void saveRemoveLog() {
        if (isClassGraph(removeGraph)) {
            HistoryManager.saveHistory(MR3Constants.HistoryType.DELETE_CLASS, removeCells);
        } else if (isPropertyGraph(removeGraph)) {
            HistoryManager.saveHistory(MR3Constants.HistoryType.DELETE_ONT_PROPERTY, removeCells);
        }
    }
    ...
    private boolean removeCells() {
        ...
        if (isInstanceGraph(removeGraph)) {
            removeGraph.removeCellsWithEdges(removeCells);
            HistoryManager.saveHistory(MR3Constants.HistoryType.DELETE_RDF, removeCells);
            return true;
        }
    ...
    public void applyLayout(MR3Constants.GraphType graphType) {
     ...
     * @param graphType
     */
    private void applyVGJRDFLayout(MR3Constants.GraphType graphType) {
        removeTypeCells();
        ...
        if (graphType == MR3Constants.GraphType.Instance && isShowTypeCell()) {
            addTypeCells();
        }
    }
    ...
    private void applyJGraphRDFLayout() {
        removeTypeCells();
        treeLayout.performJGraphTreeLayout(getInstanceGraph(), GraphLayoutUtilities.getJGraphRDFLayoutDirection(),
