...
public class GPConverter {
    ...
    public static String toGXL(RDFGraph graph, Object[] cells) {
        ...
        for (Object cell : cells) if (graph.isVertex(cell)) hash.put(cell, hash.size());

        // Convert Nodes
        for (Object node : hash.keySet()) {
            gxl += vertexGXL(graph, hash.get(node), node);
        }

        // Convert Edges
        int edges = 0;
        for (Object cell : cells) if (RDFGraph.isEdge(cell)) gxl += edgeGXL(graph, edges++, cell);
    ...
    public static String toGraphviz(RDFGraph graph, Object[] cells) {
        ...
        StringBuilder gv = new StringBuilder("/* Graphviz file generated by " + "JGraph - " + date + " */"
                + "\n\ndigraph G {");

        // Create external keys for nodes
        for (Object cell1 : cells) if (graph.isVertex(cell1)) hash.put(cell1, hash.size());
        ...
        // Process Edges
        int edges = 0;
        for (Object cell : cells) if (RDFGraph.isEdge(cell)) gv.append(edgeGraphviz(graph, edges++, cell));
